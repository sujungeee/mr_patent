...
def extract_patent_data(rtf_text):
    ...
    for i, chunk in enumerate(chunks):
        ...
        if app_match:
            ...
            if current_patent and 'patent_application_number' in current_patent:
                if all(current_patent.get(k) for k in ["patent_application_number", "patent_title"]):
                    # 필수 필드가 없으면 기본값 설정
                    if 'patent_summary' not in current_patent:
                        current_patent['patent_summary'] = "내용 없음"
                    if 'patent_claim' not in current_patent:
                        current_patent['patent_claim'] = "내용 없음"
                    if 'patent_ipc' not in current_patent:
                        current_patent['patent_ipc'] = "분류 없음"
                    
                    patents.append(current_patent)
            
            # 새 특허 정보 시작
            current_patent = {'patent_application_number': app_match.group(1).strip()}
            
            # 발명 제목 추출
            title_match = re.search(r'발명의명칭\s*:\s*(.+?)(?=\(|$)', chunk, re.DOTALL)
            if title_match:
                current_patent['patent_title'] = title_match.group(1).strip()
            
            # 요약 추출
            summary_match = re.search(r'\(요약\)\s*(.+?)(?=\(청구항\)|$)', chunk, re.DOTALL)
            if summary_match:
                current_patent['patent_summary'] = summary_match.group(1).strip()
            
            # 청구항 추출 (단수형으로 변경: claim)
            claims_match = re.search(r'\(청구항\)\s*(.+?)(?=IPC분류|$)', chunk, re.DOTALL)
            if claims_match:
                current_patent['patent_claim'] = claims_match.group(1).strip()
            
            # IPC 분류 추출 (patent_ipc로 변경)
            ipc_match = re.search(r'IPC분류\s*:\s*(.+)', chunk)
            if ipc_match:
                current_patent['patent_ipc'] = ipc_match.group(1).strip()
        
        # 마지막 특허 또는 IPC 정보가 다음 청크에 있는 경우 처리
        elif current_patent and 'patent_application_number' in current_patent:
            ipc_match = re.search(r'IPC분류\s*:\s*(.+)', chunk)
            if ipc_match and 'patent_ipc' not in current_patent:
                current_patent['patent_ipc'] = ipc_match.group(1).strip()
...
def prepare_corpus(patents: List[Dict[str, Any]]) -> List[str]:
    ...
    for patent in patents:
        ...
        summary = patent.get("patent_summary", "")
        claim = patent.get("patent_claim", "")
