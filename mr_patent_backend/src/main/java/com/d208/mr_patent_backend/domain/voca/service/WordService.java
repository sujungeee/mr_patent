...
public class WordService {
    ...
    public LevelWordsDTO getLevelWords(String userEmail, Byte levelId) {
        ...
        // 단어 목록 변환
        List<WordDTO> wordDTOList = convertToWordDTOList(words, user);

        return LevelWordsDTO.builder()
                .level_id(levelId)
    ...
    private List<WordDTO> convertToWordDTOList(List<Word> words, User user) {
        return words.stream()
                .map(word -> {
                    boolean isBookmarked = bookmarkRepository.existsByUserAndWord(user, word);
