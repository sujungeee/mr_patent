...
public class WordStudyController {
    ...
    public ResponseEntity<CommonResponseDto<LevelWordsDTO>> getLevelWords(
            Authentication authentication,
            @PathVariable("level_id") Byte levelId) {

        User user = securityUtils.getCurrentUser((HttpServletRequest) authentication);
        if (user == null) {
            throw new RuntimeException("인증 정보가 없거나 유효하지 않습니다.");
        }

        LevelWordsDTO levelWords = wordStudyService.getLevelWords(user.getUserEmail(), levelId);
        return ResponseEntity.ok(CommonResponseDto.success(levelWords));
    }
}